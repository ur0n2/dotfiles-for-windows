...
cd $env:systemdrive\;
if ( ([float]([Environment]::OSVersion.Version | % {"{0}.{1}" -f $_.Major,$_.Minor}) -ge 6.1) -and [int]($PSVersionTable.PSVersion.Major -ge 4) -and [int]($PSVersionTable.CLRVersion.Major -ge 4) ){write-output "[+] Version Check PASS"}else{write-output "[+] Required to above PS 4.0, above Windows 6.1 version";start-sleep -s 3;exit;};
